...
class CreateGroupesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('groupes', function (Blueprint $table) {
			...
			$table->string('mtle_reg');   							// (Matricule au régistaire des entreprises): string
			$table->string('descr'); 			
			$table->dateTime('dtcre');			
			$table->integer('dureexo');			
			$table->dateTime('dbexo');			
			$table->dateTime('cfinexo');	
			$table->string('contact')->nullable(false);				// (Le répondant est une personne du groupe de type membre): Nom & prenom & tél & couriel du répondant
